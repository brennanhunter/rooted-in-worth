...
export default function ModerationActions({
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      {busy && (
        <Loader2 className="h-4 w-4 animate-spin text-bark/50" aria-hidden="true" />
      )}
      {!isRemoved ? (
        <button
          type="button"
          disabled={busy}
          onClick={() => run(() => adminRemovePost(postId))}
        ...
        <button
          type="button"
          disabled={busy}
          onClick={() => run(() => adminRestorePost(postId))}
      ...
      <button
        type="button"
        disabled={busy}
        onClick={() => run(() => adminDismissReport(reportId))}
        className="rounded-full border border-bark/20 px-4 py-1.5 text-sm text-bark transition-colors hover:bg-bark/5 disabled:opacity-60"
      >
        Dismiss report
      </button>
